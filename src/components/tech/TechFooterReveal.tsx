...
export default function TechFooterReveal() {
  ...
  return (
    ...
    >
      <div className="fixed bottom-0 h-[600px] md:h-[500px] w-full">
        <footer id="contact" className="bg-[#0B0F19] h-full flex flex-col justify-end relative z-10 pt-16 pb-0 overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full relative z-20 mb-8">
            <div className="flex flex-col md:flex-row items-center justify-between gap-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 lg:gap-x-16 gap-y-1 text-sm text-gray-300">
                <SocialLink href="https://wa.me/6281252000220" icon={<WhatsAppIcon />} label="081252000220" />
                <SocialLink href="https://instagram.com/perkasa_networks" icon={<Instagram size={18} className="text-pink-500" />} label="perkasa_networks" />
                <SocialLink href="https://facebook.com/perkasa.networks" icon={<Facebook size={18} className="text-blue-500" />} label="perkasa networks" />
                <SocialLink href="https://tiktok.com/@perkasa.networks" icon={<TikTokIcon />} label="perkasa.networks" />
              </div>
            </div>

            {/* Copyright */}
            <div className="pt-4 mt-4 border-t border-white/5 text-center md:text-left flex flex-col md:flex-row justify-between items-center gap-4 text-xs text-gray-600">
          ...
          {/* Large PERKASA Logo - Trae Style (Background) */}
          <div className="absolute bottom-0 left-0 w-full overflow-hidden select-none pointer-events-none h-[40vw] md:h-[25vw] flex items-end justify-center z-0">
            <div className="relative w-full h-full scale-110 translate-y-[10%] opacity-30 mix-blend-overlay grayscale">
              <Image 
...
function SocialLink({ href, icon, label }: { href: string, icon: React.ReactNode, label: string }) {
  return (
    <a href={href} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 hover:text-white transition-colors group whitespace-nowrap">
