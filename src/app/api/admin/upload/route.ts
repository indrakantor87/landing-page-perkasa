...
export async function POST(req: Request) {
  try {
    ...
    if (file.size > maxBytes) return NextResponse.json({ error: 'Ukuran file maksimal 15MB' }, { status: 400 })
    ...
    const code = typeof (e as { code?: unknown })?.code === 'string' ? String((e as { code?: unknown }).code) : ''
